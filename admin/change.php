...
include_once('../crud.php');
    ...
    if ($displayopen=='0') {
    	$query="update single set displayopen='$val' where id='$id;'";
	$rst=mysqli_query($con,$query) or die( mysqli_error($con));
	echo $displayopen;
    }
    elseif ($displayopen=='1') {
    	$query="update single set displayopen='0' where id='$id;'";
	$rst=mysqli_query($con,$query) or die( mysqli_error($con));
	echo $displayopen;
    }
    ...
    $displayopen=$row['displayopen'];
    if ($displayopen=='0') {
      $query="update single set displayopen='$vals' where id='$id;'";
  $rst=mysqli_query($con,$query) or die( mysqli_error($con));
  echo $displayopen;
    }
